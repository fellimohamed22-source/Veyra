...
public class BookingMaintenanceScheduler {
  ...
  public void closeExpiredOfferWindows(){
    ...
    List<UUID> noOffer=db.queryForList(
      ...
      UUID.class);
    for(UUID id:noOffer){
      db.update("update scheduled_bookings set status='NO_OFFER',updated_at=now() where id=? and status='OPEN_FOR_OFFERS'",id);
    ...
    List<UUID> expired=db.queryForList(
      ...
      UUID.class);
    for(UUID id:expired){
      db.update("update scheduled_bookings set status='EXPIRED',updated_at=now() where id=? and status='OFFERS_RECEIVED'",id);
  ...
  public void reminders(){
    ...
    enqueueReminder(30,"BOOKING_REMINDER_30M");
  }

  private void enqueueReminder(int minutes,String template){
    String windowStart=String.valueOf(minutes-2);
    ...
    db.update(
      ...
      "'client-'||cast(sb.id as text)||'-'||?,jsonb_build_object('bookingId',cast(sb.id as text)),now() " +
      "from scheduled_bookings sb where sb.status in ('CONFIRMED','DRIVER_EN_ROUTE','DRIVER_ARRIVED') " +
      "and sb.scheduled_at between now()+cast(?||' minutes' as interval) and now()+cast(?||' minutes' as interval) " +
      "on conflict(dedupe_key) do nothing",
      template,template,windowStart,windowEnd);

    db.update(
      "insert into notifications(user_id,event_type,channel,template_code,dedupe_key,data,scheduled_for) " +
      "select d.user_id,'booking.reminder','PUSH',?," +
      "'driver-'||cast(sb.id as text)||'-'||?,jsonb_build_object('bookingId',cast(sb.id as text)),now() " +
